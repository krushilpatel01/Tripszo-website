...
$sql = "SELECT * FROM destination WHERE id = '$trip_id'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $filters['price_range'] = !empty($_POST['price_range']) ? (int)$_POST['price_range'] : '';
    $filters['destination'] = !empty($_POST['destination']) ? $conn->real_escape_string($_POST['destination']) : '';
    $filters['types'] = !empty($_POST['types']) ? array_map([$conn, 'real_escape_string'], $_POST['types']) : [];
}
...
if ($filters['price_range']) {
    $sql .= " AND trip.price <= " . (int)$filters['price_range'];
}

if ($filters['destination']) {
    $sql .= " AND destination.name = '" . $filters['destination'] . "'";
}

if ($filters['types']) {
    $conditions = array_map(function($type) {
        return "FIND_IN_SET('$type', trip.types)";
    }, $filters['types']);
...
<head>
    ...
    <title>Destination - <?php echo htmlspecialchars($row2['name']); ?></title>

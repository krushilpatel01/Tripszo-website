...
if (isset($_POST['save_update'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (move_uploaded_file($image_tmp_name, $image_folder)) {
            ...
            $query = "UPDATE `bus` SET name = ?, bus_number = ?, driver_number = ?, destination = ?, image = ? WHERE id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param('sssssi', $b_name, $b_number, $b_driver, $destination, $image_name, $trip_id);
        } else {
        ...
        $query = "UPDATE `bus` SET name = ?, bus_number = ?, driver_number = ?, destination = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param('ssssi', $b_name, $b_number, $b_driver, $destination, $trip_id);
    }

    if ($stmt->execute()) {
        header('Location: ../trip-bus.php');
        exit();
    } else {
                ...
                <div class="container-fluid">
                    ...
                    <div class="row add-destination">
                        ...
                        <form action="" method="post" enctype="multipart/form-data">
                                ...
                                <input type="number" name="bus_number" id="" placeholder="Enter Bus Number" value="<?php echo $trip['bus_number']; ?>"   required
                                    style="width:100%; margin:10px auto; padding:10px 0px; text-indent:10px; outline: none; ">
                                    
                                <label for="category" style="margin-bottom:0px;">Bus Driver Number:</label>
                                <input type="number" name="driver_number" id="" placeholder="Enter Bus Driver Contact" value="<?php echo $trip['driver_number']; ?>"  required
                                    style="width:100%; margin:10px auto; padding:10px 0px; text-indent:10px; outline: none;"> 
                                    
                                <!-- choose distination -->
                                <label for="category" style="margin-bottom:0px;">Destination:</label>
                                <select id="destination" name="destination" value="<?php echo $trip['destination']; ?>"  required
                                    style="width:100%; margin:10px auto; padding:10px 0px; text-indent:10px; outline: none;">
                                    <?php
